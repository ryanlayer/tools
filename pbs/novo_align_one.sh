...
#REF="/net/dutta_tumor/ref/novoalign/mm_ref_k14_s1"
...
then
	echo "No target file set"
	usage
	exit
fi

if test -z "$TARGET_POST"
then
	echo "No target postfix set"
	usage
	exit
fi
...
RUN_NAME="novo_$NAME"
QSUB_CMD="$QSUB -l select=1:mem=$MEM:ncpus=$THREADS -N novo -m bae -M rl6sf@virginia.edu"
